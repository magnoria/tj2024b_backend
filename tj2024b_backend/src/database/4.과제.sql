...
insert into employees (emplouee_id , firest_name , last_name, email, 
hire_date, job_id) value(101, 'John', 'Doe', 'jogndoe@example.com', '2024-6-24', 
'IT_PROG'); 
...
insert into products (product_id , product_name, category, pricem, stock_quantity
 ) value (201, 'Wireless Mouse', 'Electronics', 29.99, 150)
...
#[문제4]: products 테이블에서 price가 1000 인 제품의 product_name과 price를 조회하세요.
            ...
            select * from products where product_name = 1000;
            
#[문제5]: employees 테이블에서 employee_id가 101인 직원의 email을 'john.doe@company.com'으로 업데이트하세요.
	update emplouees set email = 'john.doe@company.com' where employee_id = 101;
#[문제6]: products 테이블에서 product_id가 201인 제품의 price를 24.99로 업데이트하세요.
	update products set price = 24.00 where product_id = 201;
#[문제7]: employees 테이블에서 employee_id가 101인 직원을 삭제하세요.
	delete from emplouees where employee_id =101;
