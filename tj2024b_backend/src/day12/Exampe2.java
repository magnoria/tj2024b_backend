...
class Person{
	...
	Person(){ }
		...
		//this.멤버변수 : 인스턴스의 멤버변수를 가리킨다.
		//this()      : 인스턴스의 생성자를 가리킨다.
		//this.메소드명() :  인스턴스의 메소드를 가리킨다. 주로 사용되지 않는다.
	
		//2. 사용자 정의 생성자 : 개발자가 매개변수를 임의로 해서 만든 생성자
	Person(String name, int age){
		this.name = name;
		this.age = age;
		//this란 현재 생성자를 호출하는 인스턴스의 멤버변수를 가리킨다.
		//this.name : 인스턴스의 멤버변수를 가리킨다.
		//name : 매개변수를 가리킨다.
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 할당된 멤버변수 : 4개,
		 	...
		 	String name; int age;
		 this.멤버변수 : 인스턴스의 멤버변수를 가리킨다.
		 this() : 인스턴스의 생정자를 가리킨다.
		 this :인스턴스의 참조값을 가리킨다.
