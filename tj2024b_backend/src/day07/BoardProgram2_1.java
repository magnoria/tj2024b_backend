...
public class BoardProgram2_1 {
	
	public static void main(String[] args) {
		
		//[5] 클래스 정의하기, new 연산자 : 객체 생성시 사용되는 연산자
		Product p1 = new Product();	p1.name="콜라"; p1.price=1000; p1.stock=10;		
		Product p2 = new Product();	p2.name="사이다"; p1.price=1500; p2.stock=10;	// 버릇마냥 price에 ""를 넣었음 주의하기
		Product p3 = new Product();	p3.name="환타"; p3.price=2000; p3.stock=10;	
			// +멤버변수는 객체 생성시 초기화를 생략하면 자동으로 기본값이 대입된다.(지역변수 아니다).
			// 정수 : 0, 실수 : 0.0, 논리 :false , 클래스 타입 : null
		...
		while (true) {
			...
			if (choose == 1) { //콜라선택
				...
				if (p2.stock >0) { //사이다의 재고가 존재하면
					p2.stock--; p2.cart++;
				}else{// 사이다의 재고가 존재하지 않으면
					System.out.println("사이다재고 부족합니다");
					
				}
				
			}else if (choose == 3) {

				if (p1.stock >0) { //환타의 재고가 존재하면
					p3.stock--; p3.cart++;
				}else{// 환타의 재고가 존재하지 않으면
					System.out.println("환타재고 부족합니다");
